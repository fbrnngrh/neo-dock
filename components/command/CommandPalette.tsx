...
type FilterType = "all" | "apps" | "projects" | "skills" | "actions"
...
export function CommandPalette({ open, onOpenChange, onExecute }: CommandPaletteProps) {
  ...
  const filteredResults = useMemo(() => {
    ...
    const results = fuzzySearch(filtered, query, (item) => [item.label, ...(item.keywords || [])].join(" "), 12)
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-start justify-center pt-20 z-50 backdrop-blur-sm">
      <div className="bg-neo-bg border-2 border-neo-border shadow-neo rounded-xl w-full max-w-3xl mx-4 overflow-hidden">
            ...
            filteredResults.map((result, index) => (
              <button
                ...
                className={`w-full flex items-start gap-4 p-4 text-left transition-all duration-150 ${
                  index === selectedIndex ? "bg-neo-fg text-neo-bg" : index % 2 === 0 ? "bg-neo-bg" : "bg-neo-bg2"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {result.description && (
                    <div className={`text-sm mb-2 ${index === selectedIndex ? "text-neo-bg/70" : "text-neo-fg-muted"}`}>
                      {result.description}
                    </div>
                  )}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div
                    className={`text-xs font-mono ${index === selectedIndex ? "text-neo-bg/70" : "text-neo-fg-muted"}`}
                  >
                    {index === selectedIndex ? "↵" : ""}
                  </div>
                </div>
